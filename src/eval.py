...
def visualize_table_structure(json_file):
    ...
    for obj in data:
        ...
        if label == 'table spanning cell' and 'row_numbers' in obj and 'column_numbers' in obj:
            ...
            plt.text(x + width/2, y + height/2, cell_info,
                    horizontalalignment='center',
                    verticalalignment='center',
                    fontsize=8)
    
    # 设置坐标轴
    plt.xlim(min(obj['bbox'][0] for obj in data) - 10,
             max(obj['bbox'][2] for obj in data) + 10)
    plt.ylim(min(obj['bbox'][1] for obj in data) - 10,
    ...
    # 设置标题
    plt.title('Table Structure Visualization')
    
    # 显示图形
    plt.show()

if __name__ == "__main__":
    visualize_table_structure('results/srS4l_0_objects.json')
